...
describe('multi-seed Generate diagnostics', () => {
  it('returns one result per seed and preserves runtime stage order', () => {
    const result = runMultiSeedGenerateDiagnostics({ seeds, width: 64, height: 32 });
    const source = generateWorldFromParams({ ...createDefaultGeneratorParams(), seed: seeds[0], width: 64, height: 32 });
    const runtimeIds = buildGenerateRuntimeStagePlan(source).stages.map((stage) => stage.id);
    expect(result.runs.map((run) => run.seed)).toEqual(seeds);
    expect(result.runs).toHaveLength(seeds.length);
    expect(result.runs[0].stages.map((stage) => stage.id)).toEqual(runtimeIds);
    expect(result.runs[0].stages.some((stage) => stage.id === 'CRUST_PROVINCE_DELTA')).toBe(false);
  });
  ...
  it('can ablate plate feature, isostatic, and bundled crust terrain stages', () => {
    const ablations: AblationStageId[] = ['PLATE_BOUNDARY_FEATURE_TERRAIN', 'ISOSTATIC_TERRAIN_RESPONSE', 'CRUST_TERRAIN_INFLUENCE'];
  ...
  it('identifies at least one known top suspect from the sample set', () => {
    const result = runMultiSeedGenerateDiagnostics({ seeds, width: 64, height: 32 });
    expect(result.topSuspectStages.some((stage) => ['SKELETON_ELEVATION', 'PLATE_BOUNDARY_FEATURE_TERRAIN', 'ISOSTATIC_TERRAIN_RESPONSE', 'CRUST_TERRAIN_INFLUENCE', 'QUALITY_PASS'].includes(stage))).toBe(true);

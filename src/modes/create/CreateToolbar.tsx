...
export default function CreateToolbar({ world, onSave }: Props) {
  ...
  return (
    <div style={{ padding: 14 }}>
      <div style={{ fontWeight: 900, fontSize: 14, marginBottom: 12 }}>Create</div>

      <div style={{ fontSize: 11, opacity: 0.65, marginBottom: 8 }}>World Name</div>
      <input
        value={name}
      ...
      <div style={{ fontSize: 11, opacity: 0.6, marginTop: 12, lineHeight: 1.35 }}>
        Editing tools (stickers, terrain brushes, polygons) will be mounted here per blueprint.
        This build keeps Create as a stable viewer + metadata editor.
      </div>
    </div>
  );
}
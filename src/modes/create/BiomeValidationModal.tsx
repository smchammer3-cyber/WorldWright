// ========================================================
// WORLDWRIGHT -- BIOME VALIDATION MODAL (V1.3)
// File: src/modes/create/BiomeValidationModal.tsx
//
// Shows realism warnings when placing biomes in unsuitable climates.
// Users can proceed with placement or cancel.
// ========================================================

import React from 'react';

export interface BiomeValidationProps {
  biomeType: string;
...
export function validateBiomePlacement(
  biomeId: number,
  temperature: number,
  rainfall: number
...
export default function BiomeValidationModal({
  biomeType,
  temperature,
  rainfall,
  warnings,
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ margin: '0 0 12px 0', fontSize: 16, color: 'rgba(100, 200, 255, 0.9)' }}>
          Place {biomeType} Biome?
        </h3>

        <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 16 }}>
          <div>Temperature: {(temperature * 100).toFixed(0)}%</div>
          <div>Rainfall: {(rainfall * 100).toFixed(0)}%</div>
        </div>
        ...
        >
          {warnings.map((w, i) => (
            <div key={i} style={{ color: w.startsWith('✓') ? 'rgba(100, 200, 100, 0.8)' : 'rgba(255, 150, 100, 0.9)' }}>
              {w}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          >
            {hasErrors ? 'Place Anyway (Override)' : 'Place'}
          </button>
        </div>
      </div>
    </div>

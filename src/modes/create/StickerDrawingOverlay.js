...
export default function StickerDrawingOverlay({ world, activeStickerTool, onStickerCreated, onCancel, }) {
    ...
    const handleCanvasClick = (e) => {
        if (!world || !activeStickerTool || !containerRef.current)
            return;
    ...
    const handleCanvasMouseMove = (e) => {
        if (draggedVertex === null || !containerRef.current)
            return;
        const rect = containerRef.current.getBoundingClientRect();
    ...
    return (_jsxs("div", { ref: containerRef, style: {
            ...
            zIndex: 1000,
            cursor: 'crosshair',
        }, children: [_jsx("canvas", { ref: canvasRef, onClick: handleCanvasClick, onMouseMove: handleCanvasMouseMove, onMouseDown: handleCanvasMouseDown, onMouseUp: handleCanvasMouseUp, onMouseLeave: handleCanvasMouseUp, style: {
                    position: 'absolute',
                    inset: 0,
                    display: 'block',
                } }), _jsxs("div", { style: {
                    ...
                    border: '1px solid rgba(100,200,255,0.3)',
                }, children: [_jsxs("div", { style: { fontWeight: 700, marginBottom: 8, color: 'rgba(100,200,255,0.95)' }, children: [activeStickerTool === 'BIOME' && '🌍 Biome Sticker', activeStickerTool === 'CULTURE' && '👥 Culture Zone', activeStickerTool === 'HEIGHT' && '⛏️ Terrain Sticker'] }), _jsx("div", { style: { opacity: 0.85, marginBottom: 10, lineHeight: 1.5 }, children: "Click to place vertices. Close polygon by clicking first vertex (green dot)." }), _jsxs("div", { style: { fontSize: 11, opacity: 0.7, marginBottom: 10 }, children: ["Vertices: ", currentPolygon.length] }), _jsxs("div", { style: { display: 'flex', gap: 8 }, children: [_jsx("button", { onClick: () => {
                                    setCurrentPolygon([]);
                                    onCancel?.();
                                }, style: {
                                    padding: '6px 12px',
                                    ...
                                    fontSize: 11,
                                    fontWeight: 600,
                                }, children: "Cancel" }), currentPolygon.length > 0 && (_jsx("button", { onClick: () => {
                                    ...
                                    fontSize: 11,
                                    fontWeight: 600,
                                }, children: "Undo Vertex" }))] })] }), showValidation && pendingSticker && (_jsx(BiomeValidationModal, { biomeType: activeStickerTool === 'BIOME' ? 'Biome' : activeStickerTool || '', temperature: pendingSticker.temperature, rainfall: pendingSticker.rainfall, warnings: pendingSticker.validation.warnings, onApply: () => {
                    const sticker = createSticker(pendingSticker.stickerId, activeStickerTool, currentPolygon, 'OVERRIDE', pendingSticker.payload);
                    if (world) {
                        applyStickerToWorld(world, sticker);
                        setCurrentPolygon([]);

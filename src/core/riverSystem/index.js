...
export function extractRiversFromHydrology(world, threshold = 100) {
    ...
    for (const sourceCell of riverCells) {
        ...
        while (safety < maxSafety) {
            ...
            if (!cell || cell.isWater || cell.flowDirection === null)
                break;
            currentIdx = cell.flowDirection;
            if (currentIdx < 0 || currentIdx >= cells.length)
                break;
        }
        if (path.length > 5) {
 ...
 */
export function addRiver(world, sourceCellIdx, mouthCellIdx) {
    const { cells, gridWidth, gridHeight } = world;
    if (sourceCellIdx < 0 || sourceCellIdx >= cells.length)
        return null;
    if (mouthCellIdx < 0 || mouthCellIdx >= cells.length)
        return null;
 ...
 */
function tracePathDownhill(world, startIdx) {
    const { cells, gridWidth, gridHeight, seaLevel } = world;
    const path = [];
 ...
 */
export function deleteRiver(world, riverId) {
 ...
 */
export function rerouteRiver(world, riverId, newPath) {

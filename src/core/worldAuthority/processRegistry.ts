...
export const WORLD_AUTHORITY_PROCESSES: readonly AuthorityProcessDefinition[] = Object.freeze([
  ...
  process('CAUSAL_STRUCTURE_MATERIAL_INTERPRETATION', 'diagnostic', ['causalRecord', 'diagnostics'], ['diagnostics'], { owner: 'CAUSAL_STRUCTURE_MATERIAL_DIAGNOSTIC', prerequisites: ['CAUSAL_CONTINENT_OCEAN_STRUCTURE_INTERPRETATION'], modes: SHADOW_ONLY, invariants: ['M1C resolves immutable source-backed partial candidates while universal threshold calibration and physical authority remain forbidden.', 'Separates deep substrate affinity, relative thickness, buoyancy, resistance, inherited grain, and future terrain-term permission candidates.', 'Never uses surfaceExposureSummary, exposed material, weathering, sediment cover, legacy solved morphology, renderer state, UI labels, or shadow-audit comparison as material evidence.', 'Never writes structureMaterialCause, landformPotentialAuthority, terrain, land/water, bathymetry, presentation, or canonical world state.'] }),
  process('CAUSAL_LANDFORM_POTENTIAL_INTERPRETATION', 'diagnostic', ['causalRecord', 'diagnostics'], ['diagnostics'], { owner: 'CAUSAL_LANDFORM_POTENTIAL_DIAGNOSTIC', prerequisites: ['CAUSAL_STRUCTURE_MATERIAL_INTERPRETATION'], modes: SHADOW_ONLY, invariants: ['L1A registers immutable response-potential, spatial-expression, and suppression contracts only; resolver implementation and research calibration are not authorized.', 'Reads only detached process fields, structural roles, and structure/material candidate records with exact lineage.', 'Potential labels describe permitted future responses and suppression, never height, elevation, terrain, coastlines, bathymetry, surface material, or land/water state.', 'Never writes landformPotentialAuthority, baseTerrain, finalTerrain, terrain, land/water, bathymetry, presentation, or canonical world state.'] }),
...
export function validateAuthorityProcessRegistry(): readonly string[] {
  ...
  if (material.writes.includes('structureMaterialCause')) errors.push('CAUSAL_STRUCTURE_MATERIAL_INTERPRETATION: M1C detached resolver cannot write structureMaterialCause before a separately approved authority phase');
  const landform = getAuthorityProcess('CAUSAL_LANDFORM_POTENTIAL_INTERPRETATION');
  if (landform.writes.includes('landformPotentialAuthority')) errors.push('CAUSAL_LANDFORM_POTENTIAL_INTERPRETATION: L1A contracts cannot write landformPotentialAuthority before a separately approved authority phase');

...
type PlanetFoundation = NonNullable<WorldBrain['planetFoundation']>;
type HeightBuild = { heights: Float32Array; seaLevel: number };
...
export function generateWorldFromParams(params: GeneratorParams): WorldBrain {
  ...
  const targetLandFraction = computeTargetLandFraction(params.styleMode, foundation.seaLevelOffset, foundation);
  const plateFeatureTerrainAllowed = allowsPlateBoundaryFeatureTerrain(foundation.geologyStack);
  const cells: Cell[] = Array.from({ length: width * height }, (_, i) => createEmptyCell(i));

  const tectonics = buildTectonicsField(width, height, cells, computePlateCount(foundation), rng, {
    plateActivity: plateFeatureTerrainAllowed ? foundation.tectonicVigor * 100 : foundation.tectonicVigor * 28,
  });

  for (let i = 0; i < cells.length; i++) {
    const cell = cells[i];
    const f = tectonics.fields[i];
  ...
  const terrain = buildFoundationTerrain({ width, height, seedUint, params, foundation, targetLandFraction });
  const liquidSurface = hasLiquidSurfaceWater(foundation);

  for (let i = 0; i < cells.length; i++) {
    const cell = cells[i];
    cell.baseHeight = terrain.heights[i];
    cell.editHeightDelta = 0;
    cell.simHeightDelta = 0;
    cell.isWater = liquidSurface && cell.baseHeight < terrain.seaLevel;
    cell.oceanDepthClass = classifyOceanDepth(cell.baseHeight, terrain.seaLevel, cell.isWater);
    cell.surfaceAge = clamp01(0.18 + foundation.thermalAge * 0.72 + deterministicJitter(seedUint, i, 17) * 0.08 - cell.volcanicActivity * 0.10);
    cell.surfaceType = initialSurfaceType(cell, foundation);
  }

  seedClimateAndBiomes(cells, width, height, terrain.seaLevel, seedUint, params, foundation);
  seedHydrology(cells, width, height);
...
function initialVolcanicActivity(f: TectonicsField, volcanismBias: number, geologyStack: PlanetFoundation['geologyStack']): number {
  const boundary = f.boundaryType === BoundaryType.CONVERGENT ? 0.48 : f.boundaryType === BoundaryType.DIVERGENT ? 0.40 : f.boundaryType === BoundaryType.TRANSFORM ? 0.12 : 0;
  const stackBoost = geologyStack === 'HOTSPOT_DOMINATED' ? 0.38 : geologyStack === 'RIFT_DOMINATED' ? 0.24 : geologyStack === 'VOLATILE_PRESSURE_SHELL' ? 0.18 : geologyStack === 'ICE_SHELL_TECTONIC' ? 0.14 : 0;
...
function hasLiquidSurfaceWater(foundation: PlanetFoundation): boolean {
  return foundation.surfaceWaterMode === 'LIQUID_SURFACE_WATER' || foundation.surfaceWaterMode === 'MIXED_LIQUID_ICE';
}

function initialSurfaceType(cell: Cell, foundation: PlanetFoundation): SurfaceType {
...
function buildFoundationTerrain(args: { width: number; height: number; seedUint: number; params: GeneratorParams; foundation: PlanetFoundation; targetLandFraction: number }): HeightBuild {
  ...
  for (let r = 0; r < height; r++) {
    ...
    const poleSoftener = smoothstep(0.90, 1.0, Math.abs(lat) / 90);
    for (let c = 0; c < width; c++) {
      ...
      heights[idx] = clamp((rawShape * relief * terrainSharpness + supportHeightBias(foundation)) * (1 - poleSoftener * 0.10), -1.4, 1.5);
    }
  }
...
function reliefForGeologyStack(geologyStack: PlanetFoundation['geologyStack'], tectonicVigor: number): number {
  switch (geologyStack) {
...
function supportHeightBias(foundation: PlanetFoundation): number {
...
function smoothHeightField(heights: Float32Array, width: number, height: number, passes: number, strength: number): void {
  for (let pass = 0; pass < passes; pass++) {
    ...
    for (let r = 0; r < height; r++) for (let c = 0; c < width; c++) {
      const idx = r * width + c;
      const north = Math.max(0, r - 1), south = Math.min(height - 1, r + 1), west = (c - 1 + width) % width, east = (c + 1) % width;
      const avg = (heights[north * width + c] + heights[south * width + c] + heights[r * width + west] + heights[r * width + east]) / 4;
      const lat = 90 - ((r + 0.5) / height) * 180;
      next[idx] = clamp(heights[idx] + (avg - heights[idx]) * strength * lerp(1, 0.74, smoothstep(0.86, 1.0, Math.abs(lat) / 90)), -1.4, 1.5);
    }
    heights.set(next);
  }
}
...
function addSubtleTerrainTexture(heights: Float32Array, width: number, height: number, seedUint: number, amount: number): void {
  for (let r = 0; r < height; r++) {
    ...
    for (let c = 0; c < width; c++) {
      const lon = ((c + 0.5) / width) * 360 - 180;
      const idx = r * width + c;
      const small = sphereFbm(shiftVec(latLonToUnitVector(lat, lon), 13.9, -9.1, 4.7), seedUint, 14.0, 2);
...
function carveNearSeaLevelStraits(heights: Float32Array, width: number, height: number, seedUint: number, seaLevel: number, amount: number): void {
  ...
  for (let r = 0; r < height; r++) {
    ...
    for (let c = 0; c < width; c++) {
      ...
      const channelNoise = sphereFbm(shiftVec(latLonToUnitVector(lat, lon), -31.0, 12.5, -6.0), seedUint, 4.4, 3);
      heights[idx] = clamp(copy[idx] - (1 - smoothstep(0.020, 0.095, Math.abs(channelNoise))) * smoothstep(0.35, 0.90, localLand) * amount, -1.4, 1.5);
    }
  }
}
...
function localFractionAboveSea(heights: Float32Array, width: number, height: number, row: number, col: number, seaLevel: number, radius: number): number {
  ...
  for (let dr = -radius; dr <= radius; dr++) {
    ...
    for (let dc = -radius; dc <= radius; dc++) {
      total++;
      if (heights[r * width + ((col + dc + width) % width)] >= seaLevel) land++;
    }
  }
  return total > 0 ? land / total : 0;
}

function computeTargetLandFraction(styleMode: GeneratorParams['styleMode'], seaBias: number, foundation: PlanetFoundation): number {
  ...
  const styleAdjust = styleMode === 'FANTASY' ? 0.045 : styleMode === 'STYLIZED' ? 0.018 : styleMode === 'ALIEN' ? 0.010 : 0;
...
function computePlateCount(foundation: PlanetFoundation): number {
...
function seedClimateAndBiomes(cells: Cell[], width: number, height: number, seaLevel: number, seedUint: number, params: GeneratorParams, foundation: PlanetFoundation): void {
...
function seedHydrology(cells: Cell[], width: number, height: number): void {
  ...
  for (const idx of order) {
    ...
    for (const n of neighborIndices8(width, height, idx)) if (cells[n].baseHeight < bestHeight) { bestHeight = cells[n].baseHeight; bestIdx = n; }
...
function computeSnowCover(temp: number, rain: number, elevation: number, heatIndex: number): number { return clamp01((1 - temp) * 0.70 + elevation * 0.18 + rain * 0.12 - heatIndex * 0.15); }
function pickBiome(temp: number, rain: number, snow: number, elev: number): number { if (snow > 0.58) return 7; if (elev > 0.60) return 6; if (temp > 0.68 && rain < 0.26) return 5; if (rain < 0.22) return 4; if (rain > 0.68 && temp > 0.52) return 3; if (rain > 0.50) return 2; return 1; }
...
function oceanProximityAt(cells: Cell[], width: number, height: number, row: number, col: number, radius: number): number {
  ...
  for (let dr = -radius; dr <= radius; dr++) {
    const r = row + dr;
    if (r < 0 || r >= height) continue;
    for (let dc = -radius; dc <= radius; dc++) { total++; if (cells[r * width + ((col + dc + width) % width)].isWater) water++; }
  }
...
function simpleRainShadow(cells: Cell[], width: number, height: number, row: number, col: number, seaLevel: number): number {
  ...
  for (let step = 1; step <= 3; step++) {
    const h = cells[row * width + ((col - step + width) % width)]?.baseHeight ?? seaLevel;
...
function sphereFbm(dir: Vec3, seed: number, frequency: number, octaves: number): number {
  ...
  for (let o = 0; o < octaves; o++) { value += valueNoise3(dir[0] * frequency, dir[1] * frequency, dir[2] * frequency, seed + o * 1013) * amp; norm += amp; frequency *= 2.02; amp *= 0.52; }
...
function valueNoise3(x: number, y: number, z: number, seed: number): number {
  ...
  for (let dx = 0; dx <= 1; dx++) for (let dy = 0; dy <= 1; dy++) for (let dz = 0; dz <= 1; dz++) {
    const wx = dx ? smooth(xf) : 1 - smooth(xf), wy = dy ? smooth(yf) : 1 - smooth(yf), wz = dz ? smooth(zf) : 1 - smooth(zf);
    sum += hash3(xi + dx, yi + dy, zi + dz, seed) * wx * wy * wz;
  }
  return sum * 2 - 1;
}

function hash3(x: number, y: number, z: number, seed: number): number { let h = seed ^ Math.imul(x, 374761393) ^ Math.imul(y, 668265263) ^ Math.imul(z, 2246822519); h = Math.imul(h ^ (h >>> 13), 1274126177); h ^= h >>> 16; return (h >>> 0) / 4294967295; }
function smooth(t: number): number { return t * t * (3 - 2 * t); }
function latLonToUnitVector(lat: number, lon: number): Vec3 { const phi = (lat * Math.PI) / 180, theta = (lon * Math.PI) / 180, cosPhi = Math.cos(phi); return [cosPhi * Math.cos(theta), Math.sin(phi), cosPhi * Math.sin(theta)]; }
function shiftVec(v: Vec3, x: number, y: number, z: number): Vec3 { return [v[0] + x, v[1] + y, v[2] + z]; }
...
function deterministicJitter(seed: number, index: number, salt: number): number { let h = seed ^ Math.imul(index + 1, 374761393) ^ Math.imul(salt + 1, 668265263); h = Math.imul(h ^ (h >>> 13), 1274126177); h ^= h >>> 16; return (h >>> 0) / 4294967295; }
...
function clampInt(value: number, lo: number, hi: number): number { return Math.floor(clamp(value, lo, hi)); }

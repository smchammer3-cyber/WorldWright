...
export function assignPlatesToCells(
  ...
  const fields = new Array(cells.length).fill(null).map(() => ({
    plateId: 0,
    plateType: PlateTypeEnum.CONTINENTAL as PlateType,
    boundaryType: BoundaryTypeEnum.NONE as BoundaryType,
    upliftRate: 0,
  }));

  if (plates.length === 0) return fields;
  ...
  const queue: { idx: number; plateId: number }[] = [...seeds];

  while (queue.length > 0) {
    const { idx, plateId } = queue.shift()!;
    if (plateMap[idx] !== -1) continue;

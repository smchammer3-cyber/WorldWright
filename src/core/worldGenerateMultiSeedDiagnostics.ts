...
import { buildGenerateRuntimeStagePlan, type GenerateRuntimeStageId } from './generateRuntimeStagePlan';
...
export type GenerateDiagnosticOptions = Partial<Pick<GeneratorParams, 'width' | 'height' | 'continentCount'>> & { seeds?: Array<string | number>; ablations?: AblationStageId[] };
export type MorphologyMetrics = { elevationQuantiles: Record<'p05' | 'p25' | 'p50' | 'p75' | 'p95', number>; landHeightStdDev: number; oceanDepthStdDev: number; slopeMean: number; slopeMax: number; slopeP95: number; roughnessMean: number; coastLength: number; coastComplexity: number; landPatchCount: number; landPatchSizeDistribution: number[]; largestLandmassShare: number; islandSizeClasses: Record<'tiny' | 'small' | 'medium' | 'large', number>; shorelineJaggedness: number };
export type HydrologyMetrics = { riverCount: number; riverCellCount: number; averageRiverLength: number; maxRiverLength: number; percentRiversReachingOcean: number; inlandDeadEndRiverCount: number; flowAccumulationConcentration: number; basinCount: number; tinyIsolatedDrainageArtifacts: number };
export type CauseLeakMetrics = { plateHeightJump: number | null; provinceHeightJump: number | null; skeletonHeightJump: number | null; plateTopGradientShare: number; provinceTopGradientShare: number; skeletonTopGradientShare: number; crustThicknessCorrelation: number; crustAgeCorrelation: number; continentalityCorrelation: number; shelfStrengthCorrelation: number; provincePatchCount: number; provinceLabelEntropy: number; skeletonPatchCount: number; skeletonLabelEntropy: number; heightNeighborAutocorrelation: number; provinceNeighborAutocorrelation: number };
...
export type MultiSeedGenerateDiagnostics = { seeds: string[]; grid: string; runs: Array<{ seed: string; stages: GenerateStageSnapshot[]; finalMorphology: MorphologyMetrics; finalHydrology: HydrologyMetrics; finalCauseLeak: CauseLeakMetrics; ablations: AblationResult[] }>; rankings: Record<'plateImprintIncreases' | 'provinceImprintIncreases' | 'skeletonImprintIncreases' | 'reliefDecreases' | 'fragmentIncreases' | 'fragmentRepairs' | 'worstRepairBenefitVsImprintCost', StageDeltaRanking[]>; topSuspectStages: GenerateStageId[] };
...
export function runMultiSeedGenerateDiagnostics(options: GenerateDiagnosticOptions = {}): MultiSeedGenerateDiagnostics {
  ...
  const runs = seeds.map((seed) => {
    const params = { ...paramsBase, seed };
    const source = generateWorldFromParams(params);
    const diagnostics = computeGeneratedStageDiagnostics(source) as GenerateStageDiagnostics;
    const finalWorld = generateWorldFromParams(params);
    applyGeneratedGeographyPipeline(finalWorld);
    return { seed, stages: diagnostics.stages, finalMorphology: computeMorphologyMetrics(finalWorld), finalHydrology: computeHydrologyMetrics(finalWorld), finalCauseLeak: computeCauseLeakMetrics(finalWorld), ablations: (options.ablations ?? []).map((stage) => runAblation(params, stage, diagnostics.stages[diagnostics.stages.length - 1])) };
  });
  const rankings = aggregateRankings(runs.map((run) => run.stages));
  const topSuspectStages = Array.from(new Set([...rankings.plateImprintIncreases.slice(0, 2).map((r) => r.stage), ...rankings.provinceImprintIncreases.slice(0, 2).map((r) => r.stage), ...rankings.skeletonImprintIncreases.slice(0, 2).map((r) => r.stage), ...rankings.reliefDecreases.slice(0, 1).map((r) => r.stage)]));
...
function aggregateRankings(stageRuns: GenerateStageSnapshot[][]): MultiSeedGenerateDiagnostics['rankings'] {
  const add = (metric: (s: GenerateStageSnapshot) => number | undefined, positive: boolean) => rank(stageRuns, (s) => { const v = metric(s) ?? 0; return positive ? Math.max(0, v) : Math.max(0, -v); });
  return { plateImprintIncreases: add((s) => s.deltaFromPrevious?.plateSeamHeightRatio ?? undefined, true), provinceImprintIncreases: add((s) => s.deltaFromPrevious?.provinceSeamHeightRatio ?? undefined, true), skeletonImprintIncreases: add((s) => s.deltaFromPrevious?.skeletonSeamHeightRatio ?? undefined, true), reliefDecreases: add((s) => s.deltaFromPrevious?.landHeightStdDev, false), fragmentIncreases: add((s) => (s.deltaFromPrevious?.landComponents ?? 0) + (s.deltaFromPrevious?.mediumFragmentCount ?? 0), true), fragmentRepairs: add((s) => (s.deltaFromPrevious?.landComponents ?? 0) + (s.deltaFromPrevious?.mediumFragmentCount ?? 0), false), worstRepairBenefitVsImprintCost: rank(stageRuns, (s) => Math.max(0, (s.deltaFromPrevious?.plateSeamHeightRatio ?? 0) + (s.deltaFromPrevious?.provinceSeamHeightRatio ?? 0) + (s.deltaFromPrevious?.skeletonSeamHeightRatio ?? 0)) / (1 + Math.max(0, -(s.deltaFromPrevious?.landComponents ?? 0) - (s.deltaFromPrevious?.mediumFragmentCount ?? 0)))) };
}
...
function rank(stageRuns: GenerateStageSnapshot[][], scoreOf: (stage: GenerateStageSnapshot) => number): StageDeltaRanking[] {
  ...
  for (const stages of stageRuns) for (const stage of stages.slice(1)) { const score = scoreOf(stage); const prev = scores.get(stage.id) ?? { score: 0, count: 0 }; scores.set(stage.id, { score: prev.score + score, count: prev.count + (score > 0 ? 1 : 0) }); }
...
function replayGenerate(params: GeneratorParams, skip?: AblationStageId): WorldBrain {
  ...
  for (const stage of plan.stages) {
    if (stage.id === 'RAW_GENERATOR' || stage.id === skip) continue;
    runRuntimeStage(world, stage.id);
  }
  return world;
}
...
export function assertDiagnosticsDoNotChangeNormalGenerate(params: GeneratorParams): boolean { const before = generateWorldFromParams(params); applyGeneratedGeographyPipeline(before); runMultiSeedGenerateDiagnostics({ seeds: [params.seed], width: params.width, height: params.height, continentCount: params.continentCount }); const after = generateWorldFromParams(params); applyGeneratedGeographyPipeline(after); return JSON.stringify(before.cells.map(heightAndWater)) === JSON.stringify(after.cells.map(heightAndWater)); }
function heightAndWater(c: WorldBrain['cells'][number]) { return [c.baseHeight, c.editHeightDelta, c.simHeightDelta, c.isWater]; }

function computeMorphologyMetrics(world: WorldBrain): MorphologyMetrics { const heights = world.cells.map((_, i) => totalHeight(world, i)); const land = world.cells.filter((c) => !c.isWater).map((c) => totalHeight(world, c.index)); const ocean = world.cells.filter((c) => c.isWater).map((c) => totalHeight(world, c.index)); const slopes = neighborDeltas(world); const patches = patchSizes(world, (i) => !world.cells[i].isWater); const coast = coastEdges(world); return { elevationQuantiles: quantiles(heights), landHeightStdDev: stdDev(land), oceanDepthStdDev: stdDev(ocean), slopeMean: mean(slopes), slopeMax: Math.max(0, ...slopes), slopeP95: percentile(slopes, 0.95), roughnessMean: mean(world.cells.map((_, i) => stdDev(neighborIndices4(world, i).map((n) => totalHeight(world, n))))), coastLength: coast, coastComplexity: coast / Math.max(1, Math.sqrt(Math.max(1, land.length))), landPatchCount: patches.length, landPatchSizeDistribution: patches, largestLandmassShare: (patches[0] ?? 0) / Math.max(1, land.length), islandSizeClasses: { tiny: patches.filter((s) => s < 12).length, small: patches.filter((s) => s >= 12 && s < 64).length, medium: patches.filter((s) => s >= 64 && s < 256).length, large: patches.filter((s) => s >= 256).length }, shorelineJaggedness: coast / Math.max(1, patches.length) }; }
...
function neighborAutocorr(w:WorldBrain, v:number[]){ const m=mean(v); const pairs:number[]=[]; for(let i=0;i<w.cells.length;i++) for(const n of neighborIndices4(w,i)) if(n>i) pairs.push((v[i]-m)*(v[n]-m)); const sd=stdDev(v); return sd<=1e-9?0:mean(pairs)/(sd*sd); }

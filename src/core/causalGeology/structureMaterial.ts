...
export function validateStructureMaterialState(value: unknown): asserts value is StructureMaterialStateV1 {
  ...
  assertSourceHashes(state as CreateStructureMaterialStateOptionsV1);
...
function assertSourceHashes(options: CreateStructureMaterialStateOptionsV1): void {
  assertDeterministicHash(options.sourcePremiseHash, 'Structure/material premise source');
  assertDeterministicHash(options.sourceInteriorStateHash, 'Structure/material interior source');
  assertDeterministicHash(options.sourceRegimeHistoryHash, 'Structure/material regime-history source');
  assertDeterministicHash(options.sourceGeologicSpineHash, 'Structure/material spine source');
  assertDeterministicHash(options.sourceProcessFieldProjectionHash, 'Structure/material process-field source');
  assertDeterministicHash(options.sourceContinentOceanStructureHash, 'Structure/material structural-interpretation source');
}
...
function canonicalProvinceCandidates(
  value: readonly StructureMaterialProvinceCandidateV1[],
  regionId: string,
): readonly StructureMaterialProvinceCandidateV1[] {
  ...
  const canonical = [...value].map((candidate) => {
    ...
    const rationaleIds = canonicalText(candidate.rationaleIds, `Structure/material region ${regionId} province ${candidate.provinceClass} rationale IDs`, 1);

...
function skeletonSeamDamp(world: WorldBrain, index: number): number {
  ...
  const edgeStrength = clamp01((identityEdges / neighbors.length) * 0.58 + (marginEdges / neighbors.length) * 0.24 + (gradient / neighbors.length) * 0.46);
  return lerp(1, 0.62, edgeStrength * (isCausedIsland(cell) ? 0.45 : 1));
}

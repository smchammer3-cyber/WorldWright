 ...
 */
export function generateCountries(world, count = 6) {
    ...
    const continentalPlates = new Set();
        ...
        for (let i = 0; i < cells.length; i++) {
            if (!cells[i].isWater && partition[i] !== -1) {
                ...
                for (const [nr, nc] of neighbors) {
                    ...
                    if (nIdx < 0 || nIdx >= cells.length || cells[nIdx].isWater)
                        continue;
                    ...
                    if (partition[nIdx] === -1 && cells[nIdx].plateId === myPlateId) {
                        ...
                        const heightDiff = Math.abs(cells[i].baseHeight + cells[i].editHeightDelta -
 ...
 */
function extractCountryPolygon(cells, gridWidth, gridHeight) {
    if (cells.length === 0)
        return [];
    // Get bounding box
 ...
 */
function generateCountryName(id) {
    const prefixes = [
        'Kingdom',
        'Republic',
        'Empire',

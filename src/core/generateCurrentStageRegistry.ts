...
export const CURRENT_GENERATE_STAGE_CONTRACTS: readonly CurrentGenerateStageContract[] = [
  {
    ...
    label: 'Raw generator',
    kind: 'terrain',
    registryLayerIds: ['PLATE_SHELL', 'TERRAIN_RESPONSE', 'WATER_SURFACE_STATE', 'CLIMATE_HYDROLOGY_BIOME'],
    allowedReads: ['seed', 'generator params', 'plateType', 'boundaryType', 'upliftRate', 'volcanicActivity'],
    allowedWrites: ['plateId', 'plateType', 'boundaryType', 'upliftRate', 'volcanicActivity', 'baseHeight', 'isWater', 'oceanDepthClass', 'surfaceAge', 'surfaceType', 'temperature', 'rainfall', 'snowCover', 'baseBiomeId', 'flowDirection', 'flowAccumulation'],
    mayShapeTerrain: true,
    risk: 'watch',
    note: 'Current combined bootstrap stage. It is the source terrain writer, but future work should split plate shell, feature authority, terrain response, water state, climate, hydrology, and biome response.',
  ...
  },
  {
    ...
    mayShapeTerrain: false,
    risk: 'ok',
    note: 'Derived refresh after initial skeleton terrain guidance.',
    knownViolations: [],
  },
  {
    id: 'QUALITY_PASS',
    ...
    mayShapeTerrain: true,
    risk: 'watch',
    note: 'General terrain cleanup. Must not hide upstream raw identity leaks or read raw identity while shaping terrain.',
  ...
  },
  {
    ...
    allowedReads: ['baseHeight', 'isWater', 'oceanDepthClass', 'plateType', 'boundaryType', 'upliftRate', 'volcanicActivity', 'continentality', 'shelfStrength', 'marginType'],
    ...
    knownViolations: ['crustProvince is later consumed by terrain writers in the current pipeline.'],
  },
  {
    ...
    allowedReads: ['crustThickness', 'crustAge', 'upliftRate', 'volcanicActivity', 'marginType', 'shelfStrength'],
    allowedWrites: ['baseHeight'],
    mayShapeTerrain: true,
    risk: 'problem',
    note: 'Known bad/transitional stage: current implementation switches terrain from crustProvince.',
    knownViolations: ['crustProvince -> baseHeight'],
  },
  {
    ...
    allowedReads: ['baseHeight', 'isWater', 'shelfStrength', 'marginType', 'islandCause'],
    allowedWrites: ['baseHeight'],
    mayShapeTerrain: true,
    risk: 'problem',
    note: 'Known bad/transitional stage: current implementation still uses crustProvince in coast shaping.',
    knownViolations: ['crustProvince -> baseHeight'],
  },
  {
    id: 'CRUST_COHERENCE',
    ...
    kind: 'terrain',
    registryLayerIds: ['TERRAIN_RESPONSE'],
    allowedReads: ['baseHeight', 'isWater', 'crustThickness', 'shelfStrength'],
    allowedWrites: ['baseHeight'],
    mayShapeTerrain: true,
    risk: 'watch',
    note: 'Topology/coherence cleanup. Province-label reads must be replaced with material/feature fields.',
    knownViolations: ['May still depend on province-like cleanup behavior.'],
  },
  {
    ...
    mayShapeTerrain: true,
    risk: 'problem',
    note: 'Known bad/transitional stage: late second skeleton terrain pass after skeleton already shaped terrain once.',
    knownViolations: ['late skeleton -> baseHeight', 'double-applied skeleton authority risk'],
  },
  {
  ...
  },
  {
    ...
    kind: 'feature-material',
    registryLayerIds: ['CRUST_MATERIAL'],
    allowedReads: ['baseHeight', 'isWater', 'plateType', 'boundaryType', 'continentality', 'shelfStrength', 'marginType'],
    allowedWrites: ['crustThickness', 'crustAge', 'crustProvince'],
    mayShapeTerrain: false,
    risk: 'watch',

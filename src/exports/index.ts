...
export type ExportProfile = 'UE5' | 'UNITY' | 'GODOT' | 'RAW';
 ...
 */
async function exportUE5(world: WorldBrain): Promise<ExportResult> {
 ...
 */
async function exportRaw(world: WorldBrain): Promise<ExportResult> {
  const files = new Map<string, Blob>();
  const gw = world.gridWidth;
  const gh = world.gridHeight;
...
function describeFile(filename: string): string {
  if (filename === 'heightmap.raw') return '16-bit big-endian heightmap (row-major)';
  if (filename === 'heightmap.f32') return '32-bit float heightmap (row-major)';
  if (filename.startsWith('biome_masks/')) return 'Per-biome 8-bit mask (PNG)';
  if (filename.startsWith('country_masks/')) return 'Per-country 8-bit mask (PNG)';
  if (filename === 'river_splines.json') return 'River polyline data (JSON)';
  if (filename === 'world_manifest.json' || filename === 'manifest.json') return 'Export metadata manifest';
  if (filename === 'world.json') return 'Full world data (JSON)';
  return 'Export file';
}
